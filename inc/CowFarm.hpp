...
public:
    CowFarm(const string &Name, const string &Address, int cowNumbers, double dailyMilkProduction)
        ...
        {
            cout << "[#COW_FARM_INFO]: Ferma de vaci cu numele " << Name << " si adresa " << Address << " a fost creata." << endl
                 << "[#COW_FARM_INFO]: Nr. vaci = " << cowNumbers << " vaci si productia zilnica de lapte = " << dailyMilkProduction << endl;
        }
    ~CowFarm();
